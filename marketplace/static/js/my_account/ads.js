...
(() => {
  ...
  // Apply locks WITHOUT pointer-events-none (so hover tooltip works)
  function applyEditDeleteLocks() {
    const list = getList();
    if (!list) return;
    ...
    list.querySelectorAll(".request-row").forEach((row) => {
      ...
      if (featured) {
        if (editLink) {
          editLink.classList.add("opacity-40", "cursor-not-allowed");
  ...
  if (!window.__requestsSelectHighlightWrapped) {
    ...
    window.selectHighlightPackage = async (days, cost) => {
      ...
      if (data && data.ok) {
        ...
        applyEditDeleteLocks(); // ✅ now featured => edit/delete locked
  ...
  /* =========================================================
     ...
     - block edit/delete when featured (hover still works)
  ========================================================= */
  function onRequestsClick(e) {
    ...
    if (editLink && list.contains(editLink)) {
      const id = Number(editLink.getAttribute("data-id"));
      const row = getRow(id);
      ...
      // allow normal navigation (do NOT force location.href)
      return;
    }
    ...
    if (action === "delete") {
      ...
      if (btn.classList.contains("pointer-events-none")) return; // pending
      if (featuredDays > 0) return; // featured locked
  ...
  function init() {
    updateCount();
    applyEditDeleteLocks(); // ✅ IMPORTANT (locks featured)
