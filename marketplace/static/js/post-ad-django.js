...
syncOtherForBlock(block);
  ...
  function nodeName(n) { return (n && (n.name || n.label || n.title || n.text)) || ""; }
  ...
  function aiDetectCategory(text) {
    ...
    for (const [cat, keywords] of Object.entries(aiCategories)) {
      if (keywords.some(kw => lower.includes(kw.toLowerCase()))) return cat;
    }
    return "general";
  }

  function aiBuildTemplates(title, conditionText) {
    return {
      phones: [
  ...
  aiBtn?.addEventListener("click", () => {
    ...
    const category = aiDetectCategory(title);
    const templates = aiBuildTemplates(title, condTxt);
    ...
    setTimeout(() => descField.classList.remove("ring-2", "ring-[var(--rukn-orange)]"), 800);
